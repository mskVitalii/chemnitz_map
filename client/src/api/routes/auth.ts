...
export const auth = (
  ...
  return {
    claims: async () => (await ApiClient("Check user claims")).get(`/claims`),

    login: async (data: ILoginData) =>
      (await ApiClient("Login user")).post(`/login`, data),

    logout: async () => (await ApiClient("Logout")).post(`/logout`),

    refresh: async () =>
      (await ApiClient("Get refresh token")).get(`/refresh_token`)
  };
};
